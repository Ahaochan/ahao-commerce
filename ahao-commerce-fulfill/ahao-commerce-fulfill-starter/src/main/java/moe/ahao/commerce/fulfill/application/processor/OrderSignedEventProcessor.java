...
public class OrderSignedEventProcessor extends AbstractAfterFulfillEventProcessor {
    ...
    protected boolean doBizProcess(TriggerOrderAfterFulfillEvent event) {
        ...
        if (!Objects.equals(OrderFulfillStatusEnum.DELIVERY.getCode(), orderFulfill.getStatus())) {
            log.info("履约单无法签收! orderId={}", orderFulfill.getOrderId());
            return false;
        }

        // 更新履约单状态
        OrderFulfillOperateTypeEnum operateTypeEnum = OrderFulfillOperateTypeEnum.SIGN_ORDER;
    ...
    protected String buildMsgBody(TriggerOrderAfterFulfillEvent event) {
        ...
        // 订单已签收事件
        OrderSignedEvent signedWmsEvent = (OrderSignedEvent) event.getWmsEvent();
        ...
        OrderEvent<OrderSignedEvent> orderEvent = buildOrderEvent(orderId, OrderStatusChangeEnum.ORDER_SIGNED,
                signedWmsEvent, OrderSignedEvent.class);
