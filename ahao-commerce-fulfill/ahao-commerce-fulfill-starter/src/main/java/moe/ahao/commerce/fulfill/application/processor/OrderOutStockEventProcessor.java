...
public class OrderOutStockEventProcessor extends AbstractAfterFulfillEventProcessor {
    ...
    protected boolean doBizProcess(TriggerOrderAfterFulfillEvent event) {
        ...
        if (!Objects.equals(OrderFulfillStatusEnum.FULFILL.getCode(), orderFulfill.getStatus())) {
            log.info("履约单无法出库! orderId={}", orderFulfill.getOrderId());
            return false;
        }

        // 更新履约单状态
        OrderFulfillOperateTypeEnum operateTypeEnum = OrderFulfillOperateTypeEnum.OUT_STOCK_ORDER;
    ...
    protected String buildMsgBody(TriggerOrderAfterFulfillEvent event) {
        ...
        // 订单已出库事件
        OrderOutStockEvent outStockEvent = (OrderOutStockEvent) event.getWmsEvent();
        ...
        OrderEvent<OrderOutStockEvent> orderEvent = buildOrderEvent(orderId, OrderStatusChangeEnum.ORDER_OUT_STOCKED,
                outStockEvent, OrderOutStockEvent.class);
